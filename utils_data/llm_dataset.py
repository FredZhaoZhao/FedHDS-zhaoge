...
class LLMDataset(Dataset):
    def __init__(self,
        ...
        if dataset == 'alpaca':
            ...
            # If a concrete path is passed in, use it directly.
            # Otherwise, keep backward compatibility with the symbolic name "dolly".
            actual_path = dataset
            if dataset == 'dolly':
                actual_path = os.path.join('data', 'databricks-dolly-15k.jsonl')
            actual_path = os.path.expanduser(actual_path)
            if not os.path.isabs(actual_path):
                project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
                resolved_path = os.path.join(project_root, actual_path)
                if os.path.exists(resolved_path):
                    actual_path = resolved_path

            if not os.path.exists(actual_path):
                raise FileNotFoundError(f"Dolly dataset file not found: {actual_path}")

            print(f"--- 正在从这里读取数据: {actual_path} ---")
            list_data_dict = load_jsonl(actual_path,
                                        instruction='instruction',
                                        input='context',
                                        output='response',
                                        category='category')
