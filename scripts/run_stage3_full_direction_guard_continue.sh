...
ROOT="formal_cloud_results/stage3_full_direction_guard_20260504"
...
echo "[INFO] Stage 3 continuation starts at $(date)"
echo "[INFO] ROOT=$ROOT"
echo "[INFO] This script keeps completed fl/fedhds/forget_batch_hessian results and continues retained-set cases."
...
tar -czf stage3_full_direction_guard_20260504.tar.gz \
  "$ROOT" \
  main.py \
  server.py \
  client.py \
  scripts/run_stage3_full_direction_guard.sh \
  scripts/run_stage3_full_direction_guard_continue.sh \
  scripts/summarize_experiments.py \
  scripts/make_paper_table.py \
  scripts/plot_experiments.py

echo "[DONE] Stage 3 full ablation continuation finished at $(date)"
echo "[DONE] Archive: /root/autodl-tmp/FedHDS-zhaoge/stage3_full_direction_guard_20260504.tar.gz"
